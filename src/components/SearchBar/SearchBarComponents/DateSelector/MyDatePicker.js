...
export default class MyDatePicker extends React.Component {
    ...
    setDate =dateData=> {
        let selectedDay = new Date(dateData.year, dateData.month-1, dateData.date).getTime().toString();
    ...
    getDate(){
        this.props.onDateSelection(this.state.selectedDay);
    }
    ...
    renderCalendar() {
        let days = this.state.monthDetails.map((day, index)=> {
            return (
                <div className={'c-day-container ' + (day.month !== 0 ? ' disabled' : '') + (this.isPastDay(day) ? ' disabled' : '' ) +
                (this.isCurrentDay(day) ? ' highlight' : '') + (this.isSelectedDay(day) ? ' highlight-green' : '')} key={index}>
                    <div className='cdc-day'>
                        <span onClick={()=>{this.onDateClick(day); this.props.onDateSelection(day.timestamp);}}>
                            {day.date}
                        </span>
                    </div>
                </div>
        ...
        return (
            <div className='MyDatePicker'>
                <div className='mdp-input'  onClick={()=> this.showDatePicker(true)}>
                    <IoMdCalendar size={24} style={{marginLeft: "5px", cursor:"pointer"}}/>
                    <input type='date' onChange={this.updateDateFromInput} ref={inputRef} className="unstyled"/>
                </div>
